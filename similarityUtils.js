...
export async function computeAdvancedSimilarities(
  sentences,
  { numSimilaritySentencesLookahead = 2, logging = false, model = null } = {}
) {
  if (logging)
    console.log(
  ...
  // Use the model's createEmbedding method
  if (!model || typeof model.createEmbedding !== "function") {
    throw new Error(
      "A valid model instance must be provided to computeAdvancedSimilarities"
    );
  }

  const embeddings = await Promise.all(
    sentences.map((sentence) => model.createEmbedding(sentence))
  );
  let similarities = [];
  let similaritySum = 0;
  ...
  for (let i = 0; i < embeddings.length - 1; i++) {
    ...
    for (
      let j = i + 2;
      j <= i + numSimilaritySentencesLookahead && j < embeddings.length;
      j++
    ) {
  ...
  const variance =
    similarities.reduce((acc, sim) => acc + (sim - average) ** 2, 0) /
    similarities.length;
